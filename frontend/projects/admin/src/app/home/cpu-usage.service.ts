...
export class CpuUsageService {
  ...
  getCpuUsagePerc(): Observable<string> {
    return this.httpClient.get<ActuatorResponse>('/actuator/metrics/system.cpu.usage')
      .pipe(
        map((response) => {
  ...
  getCpuCount(): Observable<number> {
    return this.httpClient.get<ActuatorResponse>('/actuator/metrics/system.cpu.count')
      .pipe(
