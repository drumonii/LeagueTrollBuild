...
Optionally, pull the PostreSQL Docker image and run a PostreSQL container from the image to use as the app database:
  ...
  docker run --name league-troll-build-pg -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=loltrollbuild -e POSTGRES_USER=postgres -p 5432:5432 -d postgres:12.2
...
Format the test json files using the IDE or an online service.
